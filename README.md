...
For supported WeChat / Weixin public-account RSS sources such as Wechat2RSS/BestBlogs, OrigRead prefers complete `content:encoded` content already present in the feed. This allows reading, translation, AI summaries and TTS without unnecessarily reopening `mp.weixin.qq.com`. Short-summary feeds can still fall back to the original page and user-visible verification when required.
...
- F-Droid / Google Play flavors do not bundle the GitHub self-update downloader or `REQUEST_INSTALL_PACKAGES` permission.
