...
import me.ash.reader.infrastructure.preference.AutoCheckUpdatesPreference
...
class AndroidApp : Application(), Configuration.Provider {
    ...
    private suspend fun checkUpdate() {
        if (!isGitHub) return
        // Application 启动早于 Compose 设置状态初始化，这里直接等待 DataStore 首个真实值，
        // 避免用户关闭自动检查后仍因默认值短暂为 ON 而误发一次 GitHub 请求。
        val autoCheckUpdates =
            AutoCheckUpdatesPreference.fromPreferences(settingsProvider.dataStore.first())
