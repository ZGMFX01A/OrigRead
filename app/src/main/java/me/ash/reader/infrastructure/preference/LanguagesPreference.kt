...
sealed class LanguagesPreference(val value: Int) : Preference() {
    ...
    fun toLocale(): Locale? = when (this) {
        UseDeviceLanguages -> resolveDeviceLocale()
        English -> Locale("en")
    ...
    companion object {
        ...
        val values = listOf(
            ...
            ChineseSimplified,
            ChineseTraditional,
            // 其他语言暂不对外提供，保留上方类型与 Locale 映射，后续需要时可恢复。
            // Arabic,
        ...
        /**
         * 跟随系统时只区分中文与非中文：简体/繁体中文分别映射，其他语言统一使用英文。
         */
        private fun resolveDeviceLocale(): Locale {
            val locale = Resources.getSystem().configuration.locales[0]
            if (locale.language != Locale.CHINESE.language) return Locale.ENGLISH
