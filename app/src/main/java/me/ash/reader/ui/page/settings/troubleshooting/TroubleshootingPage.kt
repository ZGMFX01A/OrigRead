...
fun TroubleshootingPage(onBack: () -> Unit, viewModel: TroubleshootingViewModel = hiltViewModel()) {
    ...
    val websiteRuleImportLauncher =
        rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) {
            it?.let { uri ->
                context.contentResolver.openInputStream(uri)?.use { inputStream ->
                    viewModel.importWebsiteRules(inputStream.readBytes()) { result ->
                        result.fold(
                            onSuccess = { count ->
                                Toast.makeText(
                                    context,
                                    context.getString(R.string.website_rules_imported, count),
                                    Toast.LENGTH_LONG,
                                ).show()
                            },
                            onFailure = { error ->
                                Toast.makeText(
                                    context,
                                    context.getString(
                                        R.string.website_rules_import_failed,
            ...
            LazyColumn {
                ...
                item {
                    ...
                    SettingItem(
                        ...
                        onClick = { websiteRuleImportLauncher.launch(arrayOf(MimeType.JSON, MimeType.ANY)) },
                    ) {}
                    Spacer(modifier = Modifier.height(24.dp))
                }
                item {
