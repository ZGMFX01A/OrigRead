...
/**
 * 以备份文件为准恢复用户偏好。
 * 与旧版“导入设置”的合并行为不同，这里会先清除可迁移偏好，确保备份中的默认值也能真正恢复。
 */
suspend fun String.restoreConfigurationPreferences(context: Context) {
    val gson = Gson()
    val type = object : TypeToken<Map<String, *>>() {}.type
    val deserializedMap: Map<String, Any> = gson.fromJson(this, type)

    // 先完整校验类型，避免清空现有偏好后才发现备份内容无效。
    validateConfigurationPreferencesJSONString()

    context.dataStore.edit { preferences ->
        PreferencesKey.keys.values
            .filter { it.name !in ignorePreferencesOnConfigurationBackup }
            .forEach { key ->
                when (key) {
                    is PreferencesKey.BooleanKey -> preferences.remove(key.key)
                    is PreferencesKey.FloatKey -> preferences.remove(key.key)
                    is PreferencesKey.IntKey -> preferences.remove(key.key)
                    is PreferencesKey.LongKey -> preferences.remove(key.key)
                    is PreferencesKey.StringKey -> preferences.remove(key.key)
                }
            }

        deserializedMap
            .filterKeys { it !in ignorePreferencesOnConfigurationBackup }
            .forEach { (keyString, value) ->
                when (val key = PreferencesKey.keys[keyString]) {
                    is PreferencesKey.BooleanKey -> preferences[key.key] = value as Boolean
                    is PreferencesKey.FloatKey -> preferences[key.key] = (value as Number).toFloat()
                    is PreferencesKey.IntKey -> preferences[key.key] = (value as Number).toInt()
                    is PreferencesKey.LongKey -> preferences[key.key] = (value as Number).toLong()
                    is PreferencesKey.StringKey -> preferences[key.key] = value as String
                    null -> Unit
                }
            }
    }
...
/** 导出真正属于用户的通用偏好，过滤当前账户与在线更新临时状态。 */
suspend fun Context.exportConfigurationPreferencesJSONString(): String {
...
/** 仅校验完整配置备份里的通用偏好，不修改 DataStore。 */
fun String.validateConfigurationPreferencesJSONString() {
    val gson = Gson()
    val type = object : TypeToken<Map<String, *>>() {}.type
    val deserializedMap: Map<String, Any> = gson.fromJson(this, type)
    deserializedMap
        .filterKeys { it !in ignorePreferencesOnConfigurationBackup }
        .forEach { (keyString, value) ->
            when (PreferencesKey.keys[keyString]) {
                is PreferencesKey.BooleanKey -> require(value is Boolean) { "设置 $keyString 类型错误" }
                is PreferencesKey.FloatKey,
                is PreferencesKey.IntKey,
                is PreferencesKey.LongKey -> require(value is Number) { "设置 $keyString 类型错误" }
                is PreferencesKey.StringKey -> require(value is String) { "设置 $keyString 类型错误" }
                null -> Unit
            }
        }
}
    ...
    companion object {
        ...
        val keys: MutableMap<String, DataStoreKey<*>> =
            mutableMapOf(
                ...
                autoCheckUpdates to
                    DataStoreKey(booleanPreferencesKey(autoCheckUpdates), Boolean::class.java),
                currentAccountId to
 ...
 * 完整用户配置备份中排除的运行时状态。
 *
 * 当前账户在恢复时由订阅合并逻辑保留；版本检查结果属于临时网络状态，不应跨设备迁移。
 */
val ignorePreferencesOnConfigurationBackup =
    ignorePreferencesOnExportAndImport +
        listOf(
            DataStoreKey.newVersionPublishDate,
            DataStoreKey.newVersionLog,
            DataStoreKey.newVersionSizeString,
            DataStoreKey.newVersionDownloadUrl,
            DataStoreKey.newVersionNumber,
        )
